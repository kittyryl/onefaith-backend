...
router.get("/services/:id", async (req, res) => {
  try {
    ...
    // Fetch prices
    const pricesQuery = `
      SELECT id, vehicle_type, price, is_active
      FROM carwash_service_prices
      WHERE service_id = $1
      ORDER BY price ASC
    `;
 ...
 * Manager only
 */
router.get("/admin/services", authenticateToken, requireManager, async (req, res) => {
  try {
    const servicesQuery = `
      SELECT 
    ...
    for (const service of services) {
      const pricesQuery = `
        SELECT id, vehicle_type, price, is_active, created_at
        FROM carwash_service_prices
        WHERE service_id = $1
        ORDER BY price ASC
      `;
 ...
 * Manager only
 */
router.post("/admin/services", authenticateToken, requireManager, async (req, res) => {
 ...
 * Manager only
 */
router.put("/admin/services/:id", authenticateToken, requireManager, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, category, description, display_order, is_active } = req.body;
 ...
 * Manager only
 */
router.delete("/admin/services/:id", authenticateToken, requireManager, async (req, res) => {
  try {
    const { id } = req.params;

    const deleteQuery = "DELETE FROM carwash_services_catalog WHERE id = $1 RETURNING *";
 ...
 * Manager only
 */
router.post("/admin/prices", authenticateToken, requireManager, async (req, res) => {
  try {
    const { service_id, vehicle_type, price } = req.body;

    if (!service_id || !vehicle_type || price === undefined) {
      return res.status(400).json({ error: "service_id, vehicle_type, and price are required" });
    }
    ...
    const { rows } = await pool.query(insertQuery, [service_id, vehicle_type, price]);
    ...
    if (error.code === "23505") {
      ...
      return res.status(400).json({ error: "Price for this vehicle type already exists" });
    }
 ...
 * Manager only
 */
router.put("/admin/prices/:id", authenticateToken, requireManager, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const { rows } = await pool.query(updateQuery, [vehicle_type, price, is_active, id]);
 ...
 * Manager only
 */
router.delete("/admin/prices/:id", authenticateToken, requireManager, async (req, res) => {
  try {
    const { id } = req.params;

    const deleteQuery = "DELETE FROM carwash_service_prices WHERE id = $1 RETURNING *";
    const { rows } = await pool.query(deleteQuery, [id]);
